...
def generate_predict_seq(predict_format, data, tokenizer, seq_len):
    if predict_format == "2x":
        tokens_a = list(data["seq"])  # 保持为单个字符的列表
        tokens_b = list(data["smile"])  # 保持为SMILES序列字符列表
        id = str(data["id"])
    ...
    tokens_a = tokenizer.tokenize(tokens_a,is_smile=False)  # 传入字符列表
    tokens_b = tokenizer.tokenize(tokens_b, is_smile=True)  # 传入SMILES字符列表，期望生成形如 ['SM_O', 'SM_)', 'SM_N'] 的结果
...
def run_regress():
    ...
    if args_opt.do_train==True:
        ...
        print(f"hidden_size: {bert_net_cfg.hidden_size}")
        print(f"vocab_size: {bert_net_cfg.vocab_size}")
        print(f"num_attention_heads: {bert_net_cfg.num_attention_heads}")
        print("==============================================================")
